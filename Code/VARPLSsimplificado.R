...
# p order
# determinación del orden del var por medio
...
# esta seccion se encarga de multiplicar los datos más un intercepto (por que el test de johansen dice que
...
# como hay cointegracion existe una representación VAR del VEC
...
for(ncomp in 1:c(K*p))
{
    ...
    fore.var.pls <- ci.var.pls.boot(object = var.pls1, n.ahead = h, runs = runs, exog.new = NULL,
                                    seed = 12345, ci = 0.10)
...
data.g <- data
data.g[,4] <- data[,4] + 2 # por qué le suma dos ??
